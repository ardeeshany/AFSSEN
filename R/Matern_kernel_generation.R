#' Definition of the kernel for the Matern Process
#'
#' @description Definition of the eigenfunctions and eigenvalues of the
#' kernel for the Matern Space \eqn{H^1((a,b))}.
#'
...
Matern_kernel_generation <- function(m,ord=3,domain,
                           sigma, plot.eigen = FALSE )
{
    ...
    ## ?@A: shouldn't it be return(sigma * cosh(sigma*(b-s))*cosh(sigma*(t-a))/sinh(sigma*(sigma)))?
        ...
        K<-function(t,s){
          if(ord==3)
          return((1+sqrt(3)*abs(t-s)/sigma)*exp(-sqrt(3)*abs(t-s)/sigma))
          if(ord==5)
          return((1+(sqrt(5)*abs(t-s)/sigma)+(5*(abs(t-s)^2)/(3*sigma^2)))*exp(-sqrt(5)*abs(t-s)/sigma)) 
        }
